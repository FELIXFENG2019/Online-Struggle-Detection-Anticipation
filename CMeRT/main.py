    ...
    for epoch in range(cfg.SOLVER.START_EPOCH, cfg.SOLVER.START_EPOCH + cfg.SOLVER.NUM_EPOCHS):
        ...
        if cfg.SOLVER.ENABLE_WANDB:
            run.log({
                ...
                'train_tot_loss': losses_dict['tot']['train'] / len(data_loaders['train'].dataset),
                'train_det_cls_loss': losses_dict['det_cls']['train'] / len(data_loaders['train'].dataset),
                'train_ant_cls_loss': losses_dict['ant_cls']['train'] / len(data_loaders['train'].dataset),
                'test_tot_loss': losses_dict['tot']['test'] / len(data_loaders['test'].dataset),
                'test_det_cls_loss': losses_dict['det_cls']['test'] / len(data_loaders['test'].dataset),
                'test_ant_cls_loss': losses_dict['ant_cls']['test'] / len(data_loaders['test'].dataset),
                'det_mAP': det_result['mean_AP'],
                'ant_mAP': ant_result['mean_AP'] if cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES > 0 else -1,
                'fut_mAP': fut_result['mean_AP'] if len(fut_pred_scores) > 0 else -1,
            })

        # Save checkpoint for model and optimizer
        if epoch % cfg.SOLVER.SAVE_EVERY == 0 and epoch >= 3:
...
def do_perframe_det(cfg, model, device, logger):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(pbar, start=1):
            ...
            """
            # for random baseline 
            score_bs, score_f = 32, 31 # save the original score shape
            score = []
            # import pdb; pdb.set_trace()
            # target reshape (b, 25, 2) -> (b*25, 2)
            # target_copy = copy.deepcopy(target)
            # target_copy = target_copy.reshape(-1, cfg.DATA.NUM_CLASSES)
            # get probability from target
            # background, struggle = target_copy.sum(axis=0).tolist()
            # struggle_prob = struggle / (background + struggle)
            struggle_prob = 0.5 # for purely random baseline

            for i in range(score_bs*score_f):
                per_frame_score = np.random.random_sample()
                if per_frame_score < struggle_prob:
                    per_frame_score = 1
                score.append([1 - per_frame_score, per_frame_score])
            score = np.array(score) # shape (score_bs*score_f, cfg.DATA.NUM_CLASSES)
            score = score.reshape(-1, score_f, cfg.DATA.NUM_CLASSES)
            """
                ...
                if query_indices[0] in torch.arange(0, cfg.MODEL.LSTR.WORK_MEMORY_SAMPLE_RATE):
                    if cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES > 0:
                        for t_a in range(0, cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES):
                            ...
                            # print(full_indices.shape, t_a)
                            # print(ant_scores[session][full_indices, :, t_a].shape)
        ...
        if cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES > 0:
            ...
            for t_a in range(0, cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES):
                ...
                logger.info('Action anticipation ({:.2f}s) perframe m{}: {:.5f}'.format(
                    (t_a + 1) / int(cfg.DATA.FPS) * cfg.MODEL.LSTR.ANTICIPATION_SAMPLE_RATE,
        ...
        pkl.dump({'cfg': cfg,
                  ...
                  'det_scores': pred_scores,
                  'labels': gt_targets,
                  }, open(osp.splitext(cfg.MODEL.CHECKPOINT)[0] + '-detant.pkl', 'wb'))
