...
class LSTR(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.future_enabled = self.anticipation_num_samples > 0
        ...
        # Build anticipation queries
        if self.future_enabled:
            self.anticipation_query = nn.Embedding(self.anticipation_num_samples, self.d_model)
    ...
    def forward(self, visual_inputs, motion_inputs, memory_key_padding_mask=None):
        if self.long_enabled:
            ...
            ).transpose(0, 1)) # (batch, long, d_model) to (long, batch, d_model)
        ...
        if self.work_enabled:
            ...
            # Added for Anticipation Task. Take corresponding anticipation tokens
            if self.anticipation_num_samples > 0:
                anticipation_queries = self.pos_encoding(
                    self.anticipation_query.weight[:self.anticipation_length:self.anticipation_sample_rate, :].unsqueeze(1).repeat(1,work_memories.shape[1], 1),
