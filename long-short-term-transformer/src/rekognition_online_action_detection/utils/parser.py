...
def assert_and_infer_cfg(cfg, args):
    ...
    # Infer data info
    if cfg.DATA.DATA_NAME != 'Struggle':
    ...
    cfg.DATA.FPS = data_info['fps'] if cfg.DATA.FPS is None else cfg.DATA.FPS # This is the extacted features fps
    
    # added for Struggle Dataset
    if cfg.DATA.DATA_NAME == 'Struggle':
        # get the cfg.DATA.TRAIN_SESSION_SET and cfg.DATA.TEST_SESSION_SET
        # load the json file that contains annotations and train/val/test split
        with open(cfg.DATA.DATA_SPLIT_PATH, 'r') as f:
            data_info = json.load(f)
        activity_name = cfg.DATA.DATA_SPLIT_PATH.split('/')[-2]
        cfg.DATA.TRAIN_SESSION_SET = []
        cfg.DATA.TEST_SESSION_SET = []
        for vid_name, metadata in data_info['database'].items():
            if len(vid_name.split('-')) > 1:
                # should be cross domain generalization. 
                # vid-name: '{activity name}'-'{video name}'
                activity_name = vid_name.split('-')[0]
                video_name = vid_name.split('-')[1]
            else: 
                activity_name = activity_name
                video_name = vid_name
            
            if metadata['subset'] in cfg.DATA.TRAIN_SEUBSET:
                cfg.DATA.TRAIN_SESSION_SET.append(activity_name + '-' + video_name)
            elif metadata['subset'] in cfg.DATA.TEST_SUBSET:
    ...
    if cfg.MODEL.MODEL_NAME in ['LSTR']:
        ...
        cfg.MODEL.LSTR.LONG_MEMORY_LENGTH = cfg.MODEL.LSTR.LONG_MEMORY_SECONDS * cfg.DATA.FPS # 512*3.125=1600
        cfg.MODEL.LSTR.WORK_MEMORY_LENGTH = cfg.MODEL.LSTR.WORK_MEMORY_SECONDS * cfg.DATA.FPS # 8*3.125=25
        ...
        # Added for Anticipation Task
        cfg.MODEL.LSTR.ANTICIPATION_LENGTH = cfg.MODEL.LSTR.ANTICIPATION_SECONDS * int(cfg.DATA.FPS) # int(3.125) = 3
        cfg.MODEL.LSTR.FUTURE_LENGTH = cfg.MODEL.LSTR.FUTURE_SECONDS * int(cfg.DATA.FPS) 
        ...
        cfg.MODEL.LSTR.AGES_MEMORY_LENGTH = int(cfg.MODEL.LSTR.AGES_MEMORY_LENGTH)
        cfg.MODEL.LSTR.LONG_MEMORY_LENGTH = int(cfg.MODEL.LSTR.LONG_MEMORY_LENGTH)
        cfg.MODEL.LSTR.WORK_MEMORY_LENGTH = int(cfg.MODEL.LSTR.WORK_MEMORY_LENGTH)
        cfg.MODEL.LSTR.TOTAL_MEMORY_LENGTH = int(cfg.MODEL.LSTR.TOTAL_MEMORY_LENGTH)
