...
class Focal_Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        if self.ignore_index >= 0:
            notice_index = [i for i in range(target.shape[-1]) if i != self.ignore_index]
            output = torch.sum(-self.alpha * (1 - probs[:, notice_index] + self.eps) ** self.gama * target[:, notice_index] * logsoftmax(input[:, notice_index]), dim=1)
            ...
            output = torch.sum(-self.alpha * (1 - probs + self.eps) ** self.gama * target * logsoftmax(input), dim=1)
...
class Focal_Loss(nn.Module):
    ...
    def forward(self, input, target):
        softmax = nn.Softmax(dim=1).to(input.device)
        probs = softmax(input)
        return -self.alpha*(1-probs+self.eps)**self.gama*self.criterion(input, target)
"""
